...
def show_error_popup(parent: tk.Widget, title: str, message: str) -> None:
    ...
    Matches the visual style used elsewhere in the app (white background,
    centered on the main window, consistent fonts) instead of the plain
    system dialog used by tkinter.messagebox.showerror.

    Args:
        parent:  Any widget inside the app, used to find the main window
                 for centering and to anchor the popup correctly.
        title:   Popup window title.
        message: User-facing error message. Keep this free of raw Python
                 exception text where possible — describe what went wrong
...
class HomeTab(ctk.CTkFrame):
    ...
    def _load_dataset(self) -> None:
        ...
        try:
            ...
            show_error_popup(
                self, "Load error",
                "This file could not be read. Make sure it's a valid CSV "
                "file and try again.",
            )
            logger.log_action(f"Failed to load dataset: {e}", level="error")
            return

        # Validate required columns — must run after a successful read,
        # not inside the except block above (that was the bug: this check
        # was unreachable dead code nested under the read-failure handler).
        ...
        if missing_cols:
            ...
            logger.log_action(
                f"Rejected dataset '{filename}': missing columns "
                f"{', '.join(sorted(missing_cols))}",
                level="error",
            )
            self.df = None
...
class MotifAnalysisTab(ctk.CTkFrame):
    ...
    def _run_search(self) -> None:
        ...
        try:
            ...
            show_error_popup(
                self, "Motif Search Error",
                "The motif search could not be completed. This usually "
                "means the loaded dataset is missing required columns.",
            )
            logger.log_action(f"Motif search failed: {e}", level="error")
            return
...
class ORFAnalysisTab(ctk.CTkFrame):
    ...
    def _run_analysis(self) -> None:
        ...
        try:
            ...
            show_error_popup(
                self, "Invalid input",
                "Minimum ORF length must be an integer.",
            )
            return
        ...
        try:
            ...
            show_error_popup(
                self, "ORF Analysis Error",
                "The ORF analysis could not be completed. This usually "
                "means the loaded dataset is missing required columns.",
            )
            logger.log_action(f"ORF analysis failed: {e}", level="error")
...
class ReportTab(ctk.CTkFrame):
    ...
    def _generate_pdf(self) -> None:
        ...
        try:
            ...
            self.status_label.configure(text="PDF error.", text_color="red")
            show_error_popup(
                self, "PDF Generation Error",
                "The PDF report could not be generated. Check that all "
                "required data is available and try again.",
            )
            logger.log_action(f"PDF generation failed: {e}", level="error")
            return
    ...
    def _show_pdf_success_popup(self, pdf_path: Path) -> None:
        ...
        def on_open():
            try:
                ...
                show_error_popup(
                    self, "Could not open PDF",
                    "The PDF file could not be opened automatically. "
                    "You can still find it at the saved location.",
                )
    ...
    def _generate(self) -> None:
        ...
        try:
            ...
            self.status_label.configure(
                text="Error generating report.", text_color="red"
            )
            show_error_popup(
                self, "Report Generation Error",
                "The report could not be generated. Check that all "
                "required data is available and try again.",
            )
            logger.log_action(f"Markdown report generation failed: {e}", level="error")
            return
