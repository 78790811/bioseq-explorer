    ...
    def _load_dataset(self) -> None:
        ...
        if not filename.startswith("clean_dataset_"):
            proceed = messagebox.askyesno(
                ...
                f"Do you want to try loading it anyway?",
            )
            if not proceed:
                return
...
class QualityControlTab(ctk.CTkFrame):
    ...
    def load_data(self, df) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Could not compute QC metrics: {e}") from e
...
class StatisticsTab(ctk.CTkFrame):
    ...
    def load_data(self, df) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Could not prepare data: {e}") from e
...
class BioSeqExplorerApp(ctk.CTk):
    ...
    def on_dataset_loaded(self, df) -> None:
        ...
        Collects errors from all tabs and shows a single combined popup
        instead of multiple sequential error dialogs.
        ...
        # Load each tab and collect any errors
        for tab, name in [
            (self.qc_tab,     "Quality Control"),
            (self.motif_tab,  "Motif Analysis"),
            (self.orf_tab,    "ORF Analysis"),
            (self.stats_tab,  "Statistics"),
            (self.report_tab, "Report"),
        ]:
            try:
                tab.load_data(df)
            except Exception as e:
        ...
        # Show single combined error popup if any tabs failed
        if errors:
            self._show_load_errors(errors)

    def _show_load_errors(self, errors: list[str]) -> None:
        """Show a single popup listing all tabs that failed to load data.

        Args:
            errors: List of error strings, one per failed tab.

        Returns:
            None
        """
        popup = tk.Toplevel(self)
        popup.title("Dataset loading issues")
        ...
        tk.Label(
            header,
            text="⚠  Some analyses could not be loaded",
        ...
        tk.Label(
            popup,
            text="The following tabs encountered errors with the loaded file.\n"
                 "Make sure you loaded a clean_dataset_*.csv file from HUBA.",
            font=("Segoe UI", 11),
            fg="gray",
            justify="left",
            wraplength=480,
        ...
        # Error list
        error_frame = tk.Frame(popup, bg="#FFF8F0", relief="flat", bd=1)
        error_frame.pack(fill="x", padx=20, pady=(0, 12))
